...
//
//   - a JSON handler on stdout, so `task logs -- notification` still works; and
//   - the OTel slog bridge, which converts records to OTLP logs and (when the
//     record is emitted with a request/consume context) stamps the active trace_id
//     and span_id — the trace ↔ log join key from the correlation contract.
//
// Use the *Context logging methods (slog.InfoContext, …) inside the NATS saga
// handler (whose context carries the consume span) so the trace context propagates
// to the bridge. The bridge is scoped `shoeshop/notification` (scopeName) so the
// service's own logs share the scope with its meter and events (ARCHITECTURE.md §9).
