...
// carrying an EventName (e.g. "notification.sent"). Emitting with the saga-consume
// context lets the SDK stamp the active trace_id/span_id, so events join to their
// trace exactly like logs do (correlation contract, §"Events").
//
// This is the four-signal "E". Notification's event is notification.sent: the
// fan-out the corpus records when a terminal order triggers a customer notice.
