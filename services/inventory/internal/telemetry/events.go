...
// carrying an EventName (e.g. "inventory.reserved"). Emitting with the request or
// saga-consume context lets the SDK stamp the active trace_id/span_id, so events
// join to their trace exactly like logs do (correlation contract, §"Events").
//
// This is the four-signal "E". With the v0.3 write path these become rich: the
// inventory saga emits inventory.reserved / inventory.reservation.rejected /
// inventory.released as the checkout saga moves stock.
